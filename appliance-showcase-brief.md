...
> **v0.2 修订（2026-09-06，Leo）**：定位从"客户自助看家电"改为"**销售面对客户讲解厨房布局 + 家电安装细节的工具**"。报价不再是主界面功能。见 docs/decisions.md D12。

做一个"栖居 HABITAT"风格的单页交互产品：以一个**等距（isometric）3D 厨房**为舞台，销售旋转/缩放/点击厨房里的每台家电，讲清楚**这台机器放在哪一段柜体、要多大开口、要走什么管线、装它还差什么**。配置单仍然可以导出，但它是收尾动作，不是主界面。
...
> **v0.2 修订**：按 D12 重排优先级。前三条是产品的理由，第 4 条降级。

1. **布局是主角，家电是布局的结果**：房间按真实橱柜规则排布（高柜只在段末、转角是转角柜、灶台两侧各 ≥12" 台面、洗碗机紧邻水槽…，见 D11），每个 pin 对应一段真实柜体里的真实 SKU
2. **安装细节可见**：开口尺寸、管线走向与高度、命中的安装规则，都在场景和规格卡里直接看得到，不用翻图纸
3. **同位替换（swap-in-place）+ 尺寸合身检查**：点击冰箱 → 列出同槽位的替代机型；宽度不合身的灰掉并说明差几英寸，高度/深度不符照报不拦
4. **参数化布局模板**：L / U / Galley + island，改几个参数即时重排，越界不生成并说明原因（M3-3）
5. **双语**：EN 为主，中文切换（服务华人客群）
6. **手机可用**：单指旋转、双指缩放、点击 pin；左右栏在手机上变成底部抽屉
7. **配置单**：清单页导出 JSON + 可读摘要，mailto / 剪贴板。**不在主界面**

### 2.2 明确不做（v1）

- 不做用户自定义户型/拖拽布局（那是设计软件，不是展示产品）
- **不做自由摆柜**：布局只能通过模板参数改变。家电允许在**同一柜段内**左右滑到合法位置，跨段不允许。一旦可以把柜子拖到任意位置，D11 的规则就全部变成建议而不是规则
- 不做在线支付
- 不做 AR / 实景扫描
